...
const serviceTimes = [
  ...
  { label: 'Office Hours', time: 'Mon–Fri, 9 AM – 5 PM' },
];
        ...
        >
          <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
            {serviceTimes.map((item) => (
              <li key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '1.5rem' }}>
                <span style={{ color: 'rgba(255, 255, 255, 0.85)', fontSize: '0.8125rem', fontWeight: 500 }}>
                  {item.label}
                </span>
                <span style={{ color: 'var(--accent)', fontSize: '0.8125rem', fontWeight: 600, whiteSpace: 'nowrap' }}>
                  {item.time}
                </span>
              </li>
            ))}
